...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(65.0, 100.0),  # All numbers are approximate.
                rg.Point(57.5, 113.0),
                rg.Point(42.5, 113.0),
                rg.Point(35.0, 100.0),
                rg.Point(42.5, 87.0),
                rg.Point(57.5, 87.0)]
    circle = rg.Circle(rg.Point(50, 100), 15)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  22 blue dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 22, 'blue')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  5 slices, thin (thickness=3) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def run_test_polygon():
    ...
    # Tests 3:
    title = 'POLYGON tests 3:  15 segments with thick green lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 15, 'green', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    line1 = rg.Line(points[number_of_segments-1], points[0])
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 3:  20 blue lines on green circle, hops = 5.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'green'
    fancy_polygon(window, circle, 20, 7, 'blue', 5)
    window.close_on_mouse_click()

    # Test 5 (on another window):
    title = 'FANCY POLYGON test 5:  51 red lines on white circle, hops = 25.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    fancy_polygon(window, circle, 51, 25, 'red', 5)
    window.close_on_mouse_click()

    # Test 6 (on another window):
    title = 'FANCY POLYGON test 6:  300 black lines on yellow circle, hops = 61.'
    window = rg.RoseWindow(480, 350, title)
